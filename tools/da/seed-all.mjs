...
//
// - Skips pages we've already hand-authored (architecture, thesis,
//   nav, footer, index).
...
const VITEPRESS = path.resolve(ROOT, '../oak-chain-docs');
const TMP = '/tmp/oak-seed';
...
const SKIP = new Set([
  ...
  'README.md',
  'STYLING-PORT-README.md', // working note for theme port, not a content page
]);

// Discover source pages: top-level + guide/* + operators/*.
function listSources() {
  const out = [];
  for (const f of fs.readdirSync(VITEPRESS)) {
    if (f.endsWith('.md') && !SKIP.has(f)) out.push(f);
  }
  for (const sub of ['guide', 'operators']) {
